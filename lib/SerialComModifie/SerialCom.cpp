...
template <typename T> void SerialCom::Convert_Data_Into_Char(T data, SerialCom::typeData TYPE )
  {
    T* x = new T; 
    *x = data;
    switch(TYPE) 
...
template <typename T> void SerialCom::Convert_Data_Into_Value(char buffer[8], SerialCom::typeData type)
{
    T data = 0;
    switch (type)
    {
